    ...
    if request.session.has_key('email')  and request.session.get('role') == 'user'  and request.session.has_key('token'):  
        ...
        try:
            ...
            if user.verified_at=='True' and user.status=='1':
                ...
                forx=(overall_*2)-total_farming_roi-refferal_income
...
def register(request,pk=None):   
    ...
    if request.method=='POST':
        ...
        print(token)
        if referral=='':
            User.objects.create(username='AL'+str(int(time.time()))+'P',email=email,first_name=first_name,last_name=last_name,password=password,remember_token=token)
        else:
            try:
...
def reset_password(request,pk=None):
    if pk is not None:
        ...
        try:
            ...
            if usr.role=='user':
                return render(request,'userpages/new_password_meta.html',{'em':em,'token':token,'appdetail':appdetail})                
        except:
            print('expect part')
        ...
        if request.method=='POST':
            if 'withdraw' in request.POST:
                ...
                if float(setting.min_amount)<=float(amount) and float(setting.max_amount)>=float(amount):
                    print("Hello")
                    if float(user_wallet.roi_balance)+float(user_wallet.bonus_balance)+float(user_wallet.referral_balance)>=float(amount):
                        print("I am In")     
                        ...
                        userWithdrawls.objects.create(user_id=user_id,wallet_id=user_wallet,amount=final_amount,fees=final_fees,address=address,currency=currency,type='0',roi_amount=roi_log,bonus_amount=bonus_log,direct_amount=direct_log)
                    else:
...
def buyplan(request):
    if request.session.has_key('email')  and request.session.get('role') == 'user'  and request.session.has_key('token'):  
        ...
        if 'buyplan' in request.POST:   
            ...
            if float(amount)<float(min_amount) or float(amount)>float(max_amount):
            ...
            UserMembership.objects.create(user_id=user_id,plan_id=membership.objects.get(id=id),amount=amount,max_roi=membership.objects.get(id=id).overall_roi,status='1',next_date=next_date,price=price) 
...
def get_price(request,pk=None):
    ...
    if pk is not None:
        ...
        return JsonResponse({'status':'1','data':float(data.get('data').get('last'))*float(pk)})
    else:
        return JsonResponse({'status':'0'})
...
def principal_history(request,pk=None):
    if request.session.has_key('email')  and request.session.get('role') == 'user'  and request.session.has_key('token'):  
        try:
            d = jwt.decode(request.session.get('token'), key=KEYS, algorithms=['HS256'])
            if d.get('email')!=request.session.get('email'):
...
class dailyincome(APIView):
    def get(self,request,format=None):
        ...
        for j in principal:
            ...
            if i.user_id.farming_roi_status=='0':
                continue
            max_roi=float(i.amount)/100*float(i.plan_id.principal_amount)
            roi_left=max_roi-float(i.principal_amount)
            next_date=str(i.next_date)[:10]
            d1=datetime.strptime(next_date, r"%Y-%m-%d")
            d2=datetime.strptime(str(datetime.utcnow())[:10],r"%Y-%m-%d")
            delta=d1-d2
            ...
            if roi_left>0 :
                ...
                for k in range(delta):
                    ...
                    if roi_left<1:
                        break
                    days_=calendar.monthrange(datetime.utcnow().year, datetime.utcnow().month)[1]
                    next_roi=(float(i.amount)/100*float(i.plan_id.principal_amount_per_month))/float(days_)
                    if roi_left>=next_roi:
                        i.principal_amount=float(i.principal_amount)+next_roi
                        i.save()
                        logdate=d1+timedelta(days=k)
                        Principallogs.objects.create(plan_id=i,amount=next_roi,date=str(logdate))
                        user_wallet=wallet.objects.get(user_id=i.user_id.id)
                        user_wallet.avaliable_balance=float(user_wallet.avaliable_balance)+next_roi
                        user_wallet.bonus_balance=float(user_wallet.bonus_balance)+next_roi
                        user_wallet.save()
                        message='Roi Saved Successfully'
                        ...
                        i.status='3'
                        i.save()
                        Principallogs.objects.create(plan_id=i,amount=roi_left,date=logdate)    
                        user_wallet=wallet.objects.get(user_id=i.user_id.id)
                        user_wallet.avaliable_balance=float(user_wallet.avaliable_balance)+roi_left
                        user_wallet.bonus_balance=float(user_wallet.bonus_balance)+roi_left
                        user_wallet.save()
                        message='Roi Saved Successfully'
            else:
                i.status='3'
