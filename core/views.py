...
def principal_history(request,pk=None):
    if request.session.has_key('email')  and request.session.get('role') == 'admin'  and request.session.has_key('token'):  
        ...
        data=Principallogs.objects.all()
        return render(request,'pages/principal_history.html',{'u':request.session.get('email'),
                                                      'appdetail':appdetail,
                                                      'message':message,
                                                      'message1':message1,
                                                      'data':data,
    ...
    if request.session.has_key('email')  and request.session.get('role') == 'admin'  and request.session.has_key('token'):  
        ...
        if request.method=='POST':
            ...
            if 'editpackage' in request.POST:
                ...
                obj.max_amount=max
                obj.roi=roi
                obj.plan_period=period
                obj.overall_roi=float(roi)*float(period)
                obj.principal_amount_per_month=pm
                obj.principal_amount=pa
                obj.save()
                message='data updated successfully'
                ...
                overall=float(roi)*float(period)
                membership.objects.create(name=name,min_amount=min,max_amount=max,roi=roi,overall_roi=overall,plan_period=period,principal_amount=pa,principal_amount_per_month=pm)
